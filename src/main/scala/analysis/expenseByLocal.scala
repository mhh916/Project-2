...
class expenseByLocal  (){
  ...
  def expenseByLocal(): Unit = {
    ...
    sc.spark.sql("SELECT DISTINCT UPPER(County) AS COUNTY," +
      "ROUND(CAST(ROUND(SUM(CAST(`Sale` AS Double)) Over(PARTITION BY UPPER(County))/1,2) AS DECIMAL(10,2)) / CAST(ROUND(SUM(CAST(`VolumeSoldL` AS Double)) Over(PARTITION BY UPPER(County))/1,2) AS DECIMAL(10,2)),2)" +
    ...
    sc.spark.sql("SELECT DISTINCT `ZipCode` AS ZIPCODE," +
      "ROUND(CAST(ROUND(SUM(CAST(`Sale` AS Double)) Over(PARTITION BY `ZipCode`)/1,2) AS DECIMAL(10,2)) / CAST(ROUND(SUM(CAST(`VolumeSoldL` AS Double)) Over(PARTITION BY `ZipCode`)/1,2) AS DECIMAL(10,2)),2)" +
      "AS `Price/L(Dollars)` FROM Liquor WHERE `ZipCode` IS NOT NULL ORDER BY `Price/L(Dollars)` DESC LIMIT 5").show()

       

    println("Top 5 Cities That Spent the Most Money per Liter: ")                                                                                                     //Divisor                                                                                  
    sc.spark.sql("SELECT DISTINCT UPPER(City) AS CITY," +
      "ROUND(CAST(ROUND(SUM(CAST(`Sale` AS Double)) Over(PARTITION BY UPPER(City))/1,2) AS DECIMAL(10,2)) / CAST(ROUND(SUM(CAST(`VolumeSoldL` AS Double)) Over(PARTITION BY UPPER(City))/1,2) AS DECIMAL(10,2)),2)" +
