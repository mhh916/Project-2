...
class analysisP213(spark:SparkConnect) {
  ...
  def q4(): Unit = {
    ...
    val df2 = df1.withColumn("PPLiter (L/Per Dollar)", df1("Sale") / df1("VolumeSoldL")).withColumn("Category Name", lower(df1("CategoryName"))).groupBy("Category Name").agg(avg("PPLiter (L/Per Dollar)").cast(DecimalType(10,2)).as("AVG_Price"))
    ...
    val df3 = df1.withColumn("Category Name", lower(df1("CategoryName"))).withColumn("Volume Sold (Barrels)",df1("VolumeSoldG")/53).groupBy("Category Name").agg(sum("Volume Sold (Barrels)").cast(DecimalType(10,2)).as("Total Volume (Barrels)")).orderBy(col("Total Volume (Barrels)").desc)
    df3.show(5,false)
    }
}
